  ...
  try {
    if (status === 'running') {
      const { data, error } = await supabase
        .from('cron_logs')
        .insert({ job_name: jobName, status, message, details })
        .select('id')
        .single();
      return data?.id || null;
    } else if (logId) {
      await supabase
        .from('cron_logs')
        .update({ status, message, details, ended_at: new Date().toISOString() })
        .eq('id', logId);
      return logId;
    } else {
      const { data } = await supabase
        .from('cron_logs')
        .insert({ job_name: jobName, status, message, details, ended_at: new Date().toISOString() })
        .select('id')
        .single();
      return data?.id || null;
    }
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'fetch_all') {
      const logId = await logCronExecution(supabase, 'fetch_rss_all', 'running', 'Starting RSS fetch job');
      ...
      if (feedsError) {
        await logCronExecution(supabase, 'fetch_rss_all', 'error', feedsError.message, null, logId);
        return new Response(
          JSON.stringify({ success: false, error: feedsError.message }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      ...
      const results = [];
      let totalInsertedCount = 0; // Track total inserted items
      ...
      await logCronExecution(supabase, 'fetch_rss_all', 'success', `Fetched ${totalInsertedCount} items, processed ${totalRetelled}`, {
        feedsChecked: results.length,
        itemsFound: results.reduce((acc, r) => acc + (r.itemsFound || 0), 0), // Note: itemsFound not strictly tracked in results above but inferred
        itemsInserted: totalInsertedCount,
        itemsRetelled: totalRetelled
      }, logId);
    ...
    if (action === 'process_pending') {
      const logId = await logCronExecution(supabase, 'process_pending_news', 'running', 'Starting pending news processing');
      ...
      if (countryIds.length === 0) {
        await logCronExecution(supabase, 'process_pending_news', 'success', 'No countries with 100% retell ratio', { processed: 0 }, logId);
        return new Response(
      ...
      if (!pendingItems || pendingItems.length === 0) {
        await logCronExecution(supabase, 'process_pending_news', 'success', 'No pending items to process', { processed: 0 }, logId);
        return new Response(
      ...
      await logCronExecution(supabase, 'process_pending_news', 'success', `Processed ${totalRetelled} pending items`, {
        pendingFound: pendingItems.length,
        processed: totalRetelled, // Note: using totalRetelled as proxy for successfully processed
      ...
      // Save stats to cron_stats (remote compatibility)
      await supabase
        .from('cron_stats')
        .insert({
          ...
          items_failed: pendingItems.length - totalRetelled,
          llm_model: llmModel,
