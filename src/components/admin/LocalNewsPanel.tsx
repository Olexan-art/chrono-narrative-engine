...
export function LocalNewsPanel({ password }: { password: string }) {
  ...
  // Base Retell Generation
  const [selectedNews, setSelectedNews] = useState<string>('');
  const [lastPushedLinks, setLastPushedLinks] = useState<{ id: string; title: string; url: string }[]>([]);
  const [lastPushedScoringLinks, setLastPushedScoringLinks] = useState<{ id: string; title: string; url: string }[]>([]);
  ...
  const { data: newsForDate = [], isLoading: newsLoading, refetch: refetchNewsForDate } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('[LocalNewsPanel] Loading news for:', selectedCountry, selectedDate);
      ...
      if (error) {
        console.error('[LocalNewsPanel] Error loading news:', error);
        return [];
      }

      console.log('[LocalNewsPanel] Loaded news items:', data?.length || 0);
...
---JSON_END---`;
          ...
          if (jsonMatch && jsonMatch[1]) {
            ...
            if (fallbackMatch && fallbackMatch[1]) {
              rawJsonText = fallbackMatch[1].trim();
            } else {
              rawJsonText = responseText.trim();
            }
          }

          try {
            jsonResult = JSON.parse(rawJsonText);
          } catch (e1) {
            try {
              jsonResult = JSON.parse(cleanJSON(rawJsonText));
            } catch (e2: any) {
              // Try to aggressively extract { ... } object
              const match = rawJsonText.match(/\{[\s\S]*\}/);
              if (match) {
                jsonResult = JSON.parse(cleanJSON(match[0]));
                addLog('✓ JSON recovered successfully using fallback', 'success');
              } else {
    ...
    try {
      ...
      if (res?.success) {
        ...
        if (data && data.length > 0) {
          ...
          setLastPushedScoringLinks(links);

          links.forEach((link: any) => {
            addLog(
              <span className="flex items-center gap-1 flex-wrap">
                🔗 Прод (Scoring): <a href={link.url} target="_blank" rel="noreferrer" className="underline hover:text-green-300 ml-1 truncate max-w-[300px]">{link.url}</a>
              </span>,
              'success'
            );
          });
  ...
  return (
    <Card className="max-w-5xl mx-auto">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4 p-4 rounded-lg bg-blue-500/10 border border-blue-500/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="flex items-end gap-2">
              ...
              {isGeneratingScoring && (
                <Button variant="destructive" onClick={stop} title="Зупинити генерацію Scoring">
        ...
        {/* Last Pushed Source Scoring Links */}
        {lastPushedScoringLinks.length > 0 && (
          <div className="space-y-3 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 animate-in zoom-in-95 duration-500">
            <h4 className="text-sm font-bold flex items-center gap-2 text-blue-500">
              <CheckCircle2 className="w-4 h-4" /> Нещодавно опубліковані Scoring в Live
            </h4>
            <div className="grid gap-2">
              {lastPushedScoringLinks.map((link, idx) => (
                <div key={idx} className="flex items-center justify-between gap-3 p-2 rounded bg-blue-500/5 text-xs border border-blue-500/20 group hover:bg-blue-500/10 transition-all">
                  <div className="truncate font-medium flex items-center gap-2">
                    <span className="text-[10px] text-blue-500/50">#{idx + 1}</span>
                    {link.title}
                  </div>
                  <a
                    href={link.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-shrink-0 px-3 py-1 rounded-full bg-blue-500 text-blue-950 font-bold hover:bg-blue-400 flex items-center gap-1 transition-all"
                  >
                    Переглянути <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
            ...
            <p className="text-[10px] text-muted-foreground text-center italic opacity-60">Ці скоринги новин вже доступні користувачам на продакшені</p>
          </div>
        )}
