...
export function HeroTweets({ parts }: HeroTweetsProps) {
  // Get first 2 tweets from most recent parts that have tweets
  const tweetsWithLinks: { tweet: Tweet; partDate: string }[] = [];
  
  for (const part of parts) {
    if (tweetsWithLinks.length >= 2) break;
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 max-w-2xl mx-auto">
      {tweetsWithLinks.map(({ tweet, partDate }, i) => (
        <Link 
          key={i}
          to={`/read/${partDate}`}
          className="group block"
        >
          <div className="border border-border bg-card/30 rounded-lg p-3 hover:bg-card/50 hover:border-primary/30 transition-all">
            {/* Header */}
            ...
            {/* Content */}
            <p className="text-xs leading-relaxed mb-2 font-serif line-clamp-2">
              {tweet.content}
            </p>
